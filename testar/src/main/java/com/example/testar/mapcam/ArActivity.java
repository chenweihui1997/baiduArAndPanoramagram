    ...
    public void selectItem(Object iMapPoiItem) {
        if (iMapPoiItem instanceof PoiInfoImpl) {
            /*Toast.makeText(this, "点击poi: " + ((PoiInfoImpl) iMapPoiItem).getPoiInfo().name, Toast
                    .LENGTH_SHORT).show();*/
            MainActivityTwo.startMainActivityTwo(this,((PoiInfoImpl) iMapPoiItem).getPoiInfo().location.latitude,
                    ((PoiInfoImpl) iMapPoiItem).getPoiInfo().location.longitude,((PoiInfoImpl) iMapPoiItem).getPoiInfo().address);
        }
    }
